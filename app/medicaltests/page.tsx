...
export default function UOMPage() {
  ...
  useEffect(() => { document.title = 'Units of Measure — MedLab CRUD' }, [])

  const fetchAll = async () => {
    setFetching(true)
    const { data, error } = await supabase.from('uom').select('*').order('id')
  ...
  const openEdit = (row: UOM) => { setForm({ name: row.name, description: row.description ?? '' }); setEditingId(row.id); setError(null); setModalOpen(true) }
  ...
  const handleSave = async () => {
    ...
    const { error } = editingId
      ? await supabase.from('uom').update(payload).eq('id', editingId)
      : await supabase.from('uom').insert(payload)
    if (error) { setError(error.message); setLoading(false); return }
    setModalOpen(false); await fetchAll(); setLoading(false)
  }

  const handleDelete = async (id: number) => {
    if (!confirm('Delete this UOM record? This may affect Medical Tests that reference it.')) return
  ...
  const handleExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Units of Measure')
    XLSX.writeFile(wb, 'units_of_measure.xlsx')
  }
      ...
      <div className="min-h-screen bg-[#f0f4f8] p-6 md:p-10 font-sans">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-slate-800">Units of Measure</h1>
        ...
        {modalOpen && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4 no-print">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
              <h2 className="text-lg font-bold text-slate-800 mb-4">{editingId ? 'Edit UOM' : 'New Unit of Measure'}</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. mg/dL" maxLength={15} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                  <p className="text-slate-300 text-xs mt-1">Max 15 characters</p>
                </div>
                <div>
