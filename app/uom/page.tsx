...
const emptyForm = (): FormData => ({ name: '', description: '', iduom: '', idcategory: '', normalmin: '', normalmax: '' })
...
export default function MedicalTestsPage() {
  ...
  const fetchAll = async () => {
    ...
    const { data, error } = await supabase
      .from('medicaltests')
      .select(`id, name, description, normalmin, normalmax, uom:iduom ( id, name ), testcategories:idcategory ( id, name )`)
      .order('id')
  ...
  const openEdit = (row: MedicalTest) => {
    setForm({ name: row.name, description: row.description ?? '', iduom: row.uom?.id.toString() ?? '', idcategory: row.testcategories?.id.toString() ?? '', normalmin: row.normalmin?.toString() ?? '', normalmax: row.normalmax?.toString() ?? '' })
    setEditingId(row.id); setError(null); setModalOpen(true)
  }
  ...
  const handleExcel = () => {
    const data = rows.map((r) => ({
      ID: r.id,
      'Test Name': r.name,
      Category: r.testcategories?.name ?? '',
      Unit: r.uom?.name ?? '',
      'Normal Min': r.normalmin ?? '',
      'Normal Max': r.normalmax ?? '',
      Description: r.description ?? '',
    }))
    const ws = XLSX.utils.json_to_sheet(data)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Medical Tests')
    XLSX.writeFile(wb, 'medical_tests.xlsx')
  }
              ...
              <table className="w-full text-sm min-w-[640px]">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-slate-500 font-medium text-right no-print">Actions</th>
                  </tr>
                </thead>
                  ...
                  {rows.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {row.testcategories
                          ? <span className="inline-block bg-blue-50 text-blue-700 text-xs font-mono px-2 py-0.5 rounded border border-blue-100">{row.testcategories.name}</span>
                          : <span className="text-slate-300 italic text-xs">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {row.uom
                          ? <span className="inline-block bg-emerald-50 text-emerald-700 text-xs font-mono px-2 py-0.5 rounded border border-emerald-100">{row.uom.name}</span>
                          : <span className="text-slate-300 italic text-xs">—</span>}
                      </td>
                      <td className="px-4 py-3 font-mono text-xs text-slate-600">
                        {row.normalmin != null && row.normalmax != null ? `${row.normalmin} – ${row.normalmax}` : <span className="italic text-slate-300">—</span>}
                      </td>
        ...
        {modalOpen && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4 no-print">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
              <h2 className="text-lg font-bold text-slate-800 mb-4">{editingId ? 'Edit Medical Test' : 'New Medical Test'}</h2>
              {error && <p className="text-red-500 text-xs bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4">{error}</p>}
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Test Name <span className="text-red-400">*</span></label>
                  <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Fasting Blood Glucose" maxLength={50} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Description</label>
                  <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Optional description…" rows={2} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">Category <span className="text-red-400">*</span></label>
                    <select value={form.idcategory} onChange={(e) => setForm({ ...form, idcategory: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white">
                      <option value="">Select…</option>
                      {categoryList.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">Unit of Measure <span className="text-red-400">*</span></label>
                    <select value={form.iduom} onChange={(e) => setForm({ ...form, iduom: e.target.value })} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white">
                      <option value="">Select…</option>
                      {uomList.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" step="any" value={form.normalmin} onChange={(e) => setForm({ ...form, normalmin: e.target.value })} placeholder="e.g. 70" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">Normal Max</label>
                    <input type="number" step="any" value={form.normalmax} onChange={(e) => setForm({ ...form, normalmax: e.target.value })} placeholder="e.g. 99" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono" />
                  </div>
                </div>
              </div>
